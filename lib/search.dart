...
class _SearchState extends State<Search> {
  ...
  String _errorType(){
    switch (errorType) {
      case 0: return 'E\' necessario inserire il codice';   // empty train code
      case 1: return 'Codice Treno non valido';   //invalid train code
      case 2: return 'Errore sconosciuto';
      case 3: return '(Argomento vuoto)';   // ricerca con trainStatus=""
      case 4: return 'Errore di connessione'; // nessuna connessione ad internet
      default: return null;
    }
  }
  ...
  */
  void searchButtonClick(String trainCode) {
    ...
    getAvailableNumberOfTrain(trainCode).then(
      (numberOfTrain) {
        if(numberOfTrain == 0) {    // nessun treno
          setState(() {
            errorType = 1;
          });
          return;
        ...
        if(numberOfTrain == 1) {  // un treno
          getSpecificStationCode(trainCode).then(
            (stationCode) {
              Navigator.push(
          ...
          getMultipleTrainsType(trainCode).then(
            (type) => _showDialogMultipleTrainType(type, trainCode));
        }
      }
    ).catchError((err){
      int e = 2;
      if(err is ArgumentError) e = 3;
  ...
  void _showDialogMultipleTrainType(Map<String, String> type, String trainCode) {
    showDialog(
      context: context,
          ...
          onPressed: () {
            setState(() {
              ...
              Navigator.push(
                context, CupertinoPageRoute(
                  builder: (context) => TrainStatus(
                    trainCode: trainCode, stationCode: k)
              ));
            });
          },
          child: Text(
